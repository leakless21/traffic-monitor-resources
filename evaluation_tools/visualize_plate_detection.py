...
def generate_plate_detection_visualizations(base_path="traffic-monitor-resources/Results"):
    ...
    if not all_data:
        print("No license plate detection benchmark data found to generate visualizations.")
        return
    ...
    output_dir = os.path.join('traffic-monitor-resources', 'Evaluation_Results', 'plate_detection_visualizations')
    ...
    pytorch_detailed.to_csv(os.path.join(output_dir, "plate_pytorch_detailed_performance.csv"), index=False)
    
    # Basic performance table (for compatibility)
    pytorch_basic = pytorch_data[['Model', 'Size_MB', 'mAP50-95', 'Latency_ms_im', 'FPS']].round(3)
    pytorch_basic.to_csv(os.path.join(output_dir, "plate_pytorch_performance.csv"), index=False)
    
    print(f"Plate PyTorch detailed performance table saved to {os.path.join(output_dir, 'plate_pytorch_detailed_performance.csv')}")
    print(f"Plate PyTorch basic performance table saved to {os.path.join(output_dir, 'plate_pytorch_performance.csv')}")
    print("\nLicense Plate Detection - PyTorch Detailed Performance Table:")
    ...
    if not tensorrt_data.empty:
        ...
        tensorrt_df.to_csv(os.path.join(output_dir, "plate_tensorrt_performance.csv"), index=False)
        print(f"Plate TensorRT performance table saved to {os.path.join(output_dir, 'plate_tensorrt_performance.csv')}")
        print("\nLicense Plate Detection - TensorRT Performance Table:")
    ...
    summary_df.to_csv(os.path.join(output_dir, "plate_all_formats_summary.csv"), index=False)
    print(f"Plate all formats summary saved to {os.path.join(output_dir, 'plate_all_formats_summary.csv')}")
    ...
    plt.title('License Plate Detection: mAP50-95 Comparison Across Models and Formats', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "plate_mAP_comparison.png"), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Plate mAP comparison plot saved to {os.path.join(output_dir, 'plate_mAP_comparison.png')}")

    # Plot 2: Latency comparison
    plt.figure(figsize=(14, 8))
    sns.barplot(data=combined_df, x='Model', y='Latency_ms_im', hue='Format', palette='magma')
    plt.title('License Plate Detection: Inference Latency Comparison (Lower is Better)', fontsize=16, fontweight='bold')
    ...
    plt.legend(title='Export Format', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "plate_latency_comparison.png"), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Plate latency comparison plot saved to {os.path.join(output_dir, 'plate_latency_comparison.png')}")

    # Plot 3: FPS comparison
    plt.figure(figsize=(14, 8))
    sns.barplot(data=combined_df, x='Model', y='FPS', hue='Format', palette='plasma')
    plt.title('License Plate Detection: FPS Comparison (Higher is Better)', fontsize=16, fontweight='bold')
    ...
    plt.legend(title='Export Format', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "plate_fps_comparison.png"), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Plate FPS comparison plot saved to {os.path.join(output_dir, 'plate_fps_comparison.png')}")
    ...
    if not pytorch_data.empty and 'GFLOPs' in pytorch_data.columns:
        ...
        plt.title('License Plate Detection: Model Complexity vs Accuracy (PyTorch)', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "plate_complexity_accuracy.png"), dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Plate complexity vs accuracy plot saved to {os.path.join(output_dir, 'plate_complexity_accuracy.png')}")
    ...
    if not pytorch_subset.empty:
        ...
        plt.title('License Plate Detection: Accuracy vs Latency Trade-off (PyTorch)', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "plate_pytorch_accuracy_latency.png"), dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Plate PyTorch accuracy vs latency plot saved to {os.path.join(output_dir, 'plate_pytorch_accuracy_latency.png')}")
    ...
    if not tensorrt_data.empty:
        ...
        plt.title('License Plate Detection: Accuracy vs Latency Trade-off (TensorRT)', fontsize=16, fontweight='bold')
        plt.xlabel('Latency (ms/image)', fontsize=12)
        plt.ylabel('mAP50-95', fontsize=12)
        plt.grid(True, linestyle='--', alpha=0.7)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "plate_tensorrt_accuracy_latency.png"), dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Plate TensorRT accuracy vs latency plot saved to {os.path.join(output_dir, 'plate_tensorrt_accuracy_latency.png')}")

    # Plot 7: Model performance heatmap
    plt.figure(figsize=(12, 8))
    heatmap_data = pytorch_data.set_index('Model')[['mAP50-95', 'Latency_ms_im', 'FPS', 'Size_MB']]
    
    # Normalize the data for better visualization (higher is better for mAP and FPS, lower is better for latency and size)
    heatmap_normalized = heatmap_data.copy()
    heatmap_normalized['Latency_ms_im'] = 1 / heatmap_normalized['Latency_ms_im']  # Invert latency
    heatmap_normalized['Size_MB'] = 1 / heatmap_normalized['Size_MB']  # Invert size
    
    # Normalize to 0-1 scale
    heatmap_normalized = (heatmap_normalized - heatmap_normalized.min()) / (heatmap_normalized.max() - heatmap_normalized.min())
    
    sns.heatmap(heatmap_normalized.T, annot=True, cmap='RdYlGn', center=0.5, 
                fmt='.2f', square=True, linewidths=0.5, cbar_kws={"shrink": .8})
    plt.title('License Plate Detection: Model Performance Heatmap (PyTorch)\nGreen = Better Performance', fontsize=16, fontweight='bold')
    plt.ylabel('Metrics', fontsize=12)
    plt.xlabel('Models', fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "plate_performance_heatmap.png"), dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Plate performance heatmap saved to {os.path.join(output_dir, 'plate_performance_heatmap.png')}")

    print(f"\nAll license plate detection visualizations saved to: {output_dir}")


if __name__ == "__main__":
    generate_plate_detection_visualizations() 